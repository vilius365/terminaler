...
impl TmuxDomainState {
    ...
    fn create_pane(&self, pane: &PaneItem) -> anyhow::Result<Arc<dyn Pane>> {
        ...
        let terminal = terminaler_term::Terminal::new(
            size,
    ...
    pub fn subscribe_notification(&self) {
        ...
        mux.subscribe(move |n| {
            // Unsubscribe once our domain is gone (detach/exit), otherwise
            // every attach cycle would leak a subscriber.
            match Mux::try_get() {
                Some(mux) if mux.get_domain(domain_id).is_some() => {}
                _ => return false,
            }
