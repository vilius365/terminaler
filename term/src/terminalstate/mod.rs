...
use terminaler_cell::image::ImageData;
use terminaler_cell::UnicodeVersion;
use terminaler_escape_parser::csi::{
...
use terminaler_escape_parser::{OneBased, OperatingSystemCommand, CSI};
use terminaler_surface::{CursorShape, CursorVisibility, SequenceNo};
...
lazy_static::lazy_static! {
    static ref DB: Database = {
        let data = include_bytes!("../../../termwiz/data/terminaler");
...
impl TerminalState {
    ...
    /// Here in terminaler the terminalstate is isolated from other
    ...
    fn perform_csi_mode(&mut self, mode: Mode) {
        match mode {
            ...
            Mode::SetDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in terminaler's mux
            }
            Mode::ResetDecPrivateMode(DecPrivateMode::Code(
                DecPrivateModeCode::SynchronizedOutput,
            )) => {
            ...
            Mode::QueryDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in terminaler's mux; if we get here, then it isn't enabled,
    ...
    fn checksum_rectangle(&mut self, left: u32, top: u32, right: u32, bottom: u32) -> u16 {
        ...
        */
        ...
        // The concept of uninitialized cells in terminaler is not the same as that on VT520 or that
