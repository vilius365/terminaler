...
pub fn set_terminaler_executable() {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            std::env::set_var("TERMINALER_EXECUTABLE_DIR", dir);
        }
        std::env::set_var("TERMINALER_EXECUTABLE", exe);
    }
}
...
pub fn fixup_appimage() {
    if let Some(appimage) = std::env::var_os("APPIMAGE") {
        ...
        // `Terminaler.AppImage foo`, which is super confusing for everyone!
        ...
        // TERMINALER_EXECUTABLE_DIR is set by `set_terminaler_executable`
        // which is called before `fixup_appimage`
        if let Some(dir) = std::env::var_os("TERMINALER_EXECUTABLE_DIR") {
        ...
        /// the correct environment when it is launched via terminaler.
        /// However, if we are using the system terminaler to spawn a portable
        /// AppImage then we want these to not take effect.
        fn clean_terminaler_config_env() {
...
pub fn bootstrap() {
    ...
    // terminaler never sets WINDOWID and we don't want to inherit it from a
    // parent process.
    std::env::remove_var("WINDOWID");
    // Avoid vte shell integration kicking in if someone started
    // terminaler or the mux server from inside gnome terminal.
