...
use terminaler_input_types::Modifiers;
...
pub use terminaler_input_types::KittyKeyboardFlags;
...
impl<'a> CSIParser<'a> {
    ...
    fn parse_sgr_color(&mut self, params: &'a [CsiParam]) -> Result<ColorSpec, ()> {
        match params {
            // terminaler extension to support an optional alpha channel in the `:` form only
            [
                _,
                CsiParam::P(b':'),
                CsiParam::Integer(6),
