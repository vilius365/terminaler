...
pub const DEFAULT_WINDOW_CLASS: &str = "org.wezfurlong.terminaler";
...
pub struct StartCommand {
    ...
    /// in your terminaler configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing terminaler GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of terminaler so that you can wait for the command
    /// to complete by waiting for this terminaler process to finish.
    ...
    /// The default is "org.wezfurlong.terminaler".
    ...
    /// instance of terminaler, including error, update and ssh
    ...
    /// terminaler will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, terminaler will attach
    /// AND then spawn PROG.
    #[arg(long, requires = "domain")]
    pub attach: bool,

    /// Instead of executing your shell, run PROG.
    /// For example: `terminaler start -- bash -l` will spawn bash
