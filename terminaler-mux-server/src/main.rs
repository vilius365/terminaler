...
use terminaler_mux_server_impl::update_mux_domains_for_server;
...
pub fn spawn_listener() -> anyhow::Result<()> {
    ...
    std::env::set_var("TERMINALER_UNIX_SOCKET", unix_dom.socket_path());
    let mut listener = terminaler_mux_server_impl::local::LocalListener::with_domain(&unix_dom)?;
    thread::spawn(move || {
        listener.run();
    });
