...
use terminaler_term::{CellAttributes, Intensity};
use terminaler_toast_notification::ToastNotification;
...
impl FontConfigInner {
    ...
    fn compute_title_font(&self, config: &ConfigHandle, make_bold: bool) -> (TextStyle, f64) {
        ...
        let font_size = if cfg!(windows) { 12. } else { 12. };
            ...
            if !attr.is_synthetic && !attr.is_fallback && !loaded.contains(attr) {
                ...
                let explanation = if is_primary {
                    ...
                    format!(
                        ...
                        Note that terminaler will synthesize font_rules to select bold \
    ...
    pub fn match_style<'a>(
        ...
        let would_bright = match attrs.foreground() {
            terminaler_term::color::ColorAttribute::PaletteIndex(idx) if idx < 8 => {
