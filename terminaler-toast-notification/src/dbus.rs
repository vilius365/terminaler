            ...
            while let Some(signal) = invoked_stream.next().await {
                ...
                if args.nid == notification {
                    if let Some(url) = notif.url.as_ref() {
                        // Open the URL with the desktop's default handler.
                        // Mirrors the Windows backend, which shells out to
                        // `cmd /C start` for the same purpose.
                        if let Err(err) =
                            std::process::Command::new("xdg-open").arg(url).spawn()
                        {
                            log::error!("failed to spawn xdg-open {}: {}", url, err);
                        }
                        abort_closed.abort();
                        break;
                    }
