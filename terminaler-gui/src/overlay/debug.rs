...
pub fn show_debug_overlay(
    ...
    lua.load("terminaler = require 'terminaler'").exec()?;
    ...
    let version = config::terminaler_version();
    let triple = config::terminaler_target_triple();

    term.render(&[Change::Text(format!(
        "Debug Overlay\r\n\
         terminaler version: {version} {triple}\r\n\
