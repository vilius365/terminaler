...
// STRIPPED: terminaler_bidi removed; use bidi stub from termwiz::surface
use termwiz::surface::bidi_stub::Direction;
use terminaler_client::domain::ClientDomain;
use terminaler_font::shaper::PresentationWidth;
...
enum SubCommand {
    ...
    #[command(name = "connect", about = "Connect to terminaler multiplexer")]
...
async fn async_run_terminal_gui(
    ...
    std::env::set_var("TERMINALER_UNIX_SOCKET", unix_socket_path.clone());
    terminaler_blob_leases::register_storage(Arc::new(
        terminaler_blob_leases::simple_tempdir::SimpleTempDir::new_in(&*config::CACHE_DIR)?,
    ))?;
...
impl Publish {
    pub fn resolve(mux: &Arc<Mux>, config: &ConfigHandle, always_new_process: bool) -> Self {
        ...
        match terminaler_client::discovery::resolve_gui_sock_path(
    ...
    pub fn try_spawn(
        ...
        if let Publish::TryPathOrPublish(gui_sock) = &self {
            ...
            match terminaler_client::client::Client::new_unix_domain(None, &dom, false, &mut ui, true)
            {
                Ok(client) => {
                    ...
                    let res = block_on(executor.run(async move {
                        ...
                        if vers.config_file_path
                            != std::env::var_os("TERMINALER_CONFIG_FILE").map(Into::into)
                        {
                            *self = Publish::NoConnectNoPublish;
                            anyhow::bail!(
                    ...
                    match res {
                        Ok(res) => {
                            log::info!(
                             ...
                             Use terminaler start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn spawn_mux_server(unix_socket_path: PathBuf, should_publish: bool) -> anyhow::Result<()> {
    let mut listener =
        terminaler_mux_server_impl::local::LocalListener::with_domain(&config::UnixDomain {
    ...
    std::thread::spawn(move || {
        let name_holder;
        if should_publish {
            name_holder = terminaler_client::discovery::publish_gui_sock_path(
                &unix_socket_path,
                &crate::termwindow::get_window_class(),
            );
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running terminaler to do this.
...
fn notify_on_panic() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        if let Some(s) = info.payload().downcast_ref::<&str>() {
            fatal_toast_notification("Terminaler panic", s);
        }
        default_hook(info);
    }));
}
...
fn terminate_with_error_message(err: &str) -> ! {
    ...
    fatal_toast_notification("Terminaler Error", &err);
    std::process::exit(1);
}
...
pub fn run_ls_fonts(config: config::ConfigHandle, cmd: &LsFontsCommand) -> anyhow::Result<()> {
    use terminaler_font::parser::ParsedFont;
    ...
    let font_config = Rc::new(terminaler_font::FontConfiguration::new(
        Some(config.clone()),
    ...
    if let Some(text) = &text {
        ...
        let ft_lib = terminaler_font::ftwrap::Library::new()?;
            ...
            while let Some(info) = iter.next() {
                ...
                if config.custom_block_glyphs {
                    if let Some(block) = info.only_char.and_then(BlockKey::from_char) {
                        ...
                        println!(
                            "{:2} {:4} {:12} drawn by terminaler because custom_block_glyphs=true: {:?}",
...
fn run() -> anyhow::Result<()> {
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["terminaler-gui".to_string()];
    ...
    match sub {
        SubCommand::Start(start) => {
            ...
            terminaler_blob_leases::clear_storage();
            res
        }
