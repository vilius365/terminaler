...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let terminaler_perform_key_assignment_sel = sel!(terminalerPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    terminaler_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["Terminaler", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "Terminaler" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("Terminaler {}", config::terminaler_version()),
                            Some(terminaler_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::terminaler_version().to_string(),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        QuitApplication => CommandDef {
            brief: "Quit Terminaler".into(),
            doc: "Quits Terminaler".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://wezfurlong.org/wezterm/" => CommandDef {
                ...
                doc: "Visit the terminaler documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            ...
            "https://github.com/wez/wezterm/discussions/" => CommandDef {
                ...
                doc: "Visit terminaler's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
            ...
            "https://github.com/wez/wezterm/issues/" => CommandDef {
                ...
                doc: "Visit terminaler's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
