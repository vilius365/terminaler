...
bitflags! {
    ...
    pub struct Modifiers: u16 {
        ...
        const LEFT_ALT = 1<<5;
        const RIGHT_ALT = 1<<6;
        /// This is a virtual modifier used by terminaler
        const LEADER = 1<<7;
        const LEFT_CTRL = 1<<8;
...
impl FromDynamic for IntegratedTitleButtonStyle {
        ...
        Self: Sized,
    {
        ...
        if let terminaler_dynamic::Value::String(string) = value {
            let style = match string.as_str() {
                "Windows" => Self::Windows,
                "Gnome" => Self::Gnome,
                "MacOsNative" if cfg!(target_os = "macos") => Self::MacOsNative,
                _ => {
                    return Err(terminaler_dynamic::Error::InvalidVariantForType {
            ...
            Err(terminaler_dynamic::Error::InvalidVariantForType {
