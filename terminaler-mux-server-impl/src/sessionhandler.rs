...
use url::Url;
use terminaler_term::terminal::Alert;
use terminaler_term::StableRowIndex;
        ...
        match decoded.pdu {
            ...
            Pdu::SetClientId(SetClientId {
                ...
                if is_proxy {
                    ...
                    // on from the `terminaler cli list-clients` information
                ...
                match std::env::current_exe().context("resolving current_exe") {
                    ...
                    Ok(executable_path) => {
                        send_response(Ok(Pdu::GetCodecVersionResponse(GetCodecVersionResponse {
                            ...
                            version_string: config::terminaler_version().to_owned(),
                            executable_path,
                            config_file_path: std::env::var_os("TERMINALER_CONFIG_FILE")
                                .map(Into::into),
                        })))
                    }
                }
