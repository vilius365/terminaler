...
#[cfg(feature = "std")]
use terminaler_blob_leases::{BlobLease, BlobManager};
...
pub enum ImageDataType {
    ...
    EncodedLease(
        #[cfg_attr(
            feature = "use_serde",
            serde(with = "terminaler_blob_leases::lease_bytes")
        )]
        BlobLease,
    ),
    ...
    pub fn swap_out(self) -> Result<Self, ImageCellError> {
        match self {
            Self::EncodedFile(data) => match BlobManager::store(&data) {
                ...
                Err(terminaler_blob_leases::Error::StorageNotInit) => Ok(Self::EncodedFile(data)),
...
pub enum ImageCellError {
    ...
    #[error(transparent)]
    BlobLease(#[from] terminaler_blob_leases::Error),
