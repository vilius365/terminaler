...
impl Header {
    fn new(config: &ConfigHandle, size: PtySize, prog: &[&OsStr]) -> Self {
        ...
        env.insert(
            "TERMINALER_VERSION".to_string(),
            config::terminaler_version().to_string(),
        );
        env.insert(
            "TERMINALER_TARGET_TRIPLE".to_string(),
            config::terminaler_target_triple().to_string(),
        );
...
pub struct RecordCommand {
    ...
    /// the default_cwd defined by your terminaler configuration
    #[arg(long)]
