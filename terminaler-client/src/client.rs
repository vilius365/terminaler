...
// STRIPPED: use async_ossl::AsyncSslStream;
...
use mux::pane::PaneId;
// STRIPPED: use mux::ssh::ssh_connect_with_ui; (SSH support stripped)
use mux::Mux;
// STRIPPED: use openssl::ssl::{SslConnector, SslFiletype, SslMethod};
// STRIPPED: use openssl::x509::X509;
...
#[error(
    "Please install the same version of terminaler on both the client and server!\n\
...
impl Reconnectable {
    ...
    fn connect(
        ...
        match self.config.clone() {
            ...
            // STRIPPED: TLS and SSH domain connection removed (support stripped)
        }
    }

    /// Resolve the path to terminaler for the remote system.
    ...
    /// `path` comes from the SshDoman::remote_terminaler_path option; if set
    ...
    /// message to the user.
    fn terminaler_bin_path(path: &Option<String>) -> String {
        path.as_deref().unwrap_or("terminaler").to_string()
    }

    // STRIPPED: ssh_connect removed (SSH support stripped)
    ...
    // STRIPPED: tls_connect removed (TLS support stripped)
    // STRIPPED: try_connect removed (TLS support stripped)
}
...
impl Client {
        ...
        thread::spawn(move || {
            ...
            loop {
                if let Err(e) = client_thread(&mut reconnectable, local_domain_id, &mut receiver) {
                    ...
                    ui.title("terminaler: Reconnecting...");
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                let msg = if err.root_cause().is::<Timeout>() {
                    ...
                    format!(
                        "Please install the same version of terminaler on both \
    ...
    fn compute_unix_domain(
        ...
        match std::env::var_os("TERMINALER_UNIX_SOCKET") {
            ...
            Some(_) | None => {
                ...
                // STRIPPED: config.unix_domains removed; return error since unix domain config is gone
                anyhow::bail!(
                    "no default unix domain is configured and TERMINALER_UNIX_SOCKET \
    ...
    // STRIPPED: new_tls removed (TLS support stripped)
    // STRIPPED: new_ssh removed (SSH support stripped)
    ...
    pub async fn resolve_pane_id(&self, pane_id: Option<PaneId>) -> anyhow::Result<PaneId> {
        let pane_id: PaneId = match pane_id {
            Some(p) => p,
            None => {
                if let Ok(pane) = std::env::var("TERMINALER_PANE") {
                    pane.parse()?
                } else {
                    ...
                    if clients.is_empty() {
                        anyhow::bail!(
                            "--pane-id was not specified and $TERMINALER_PANE
