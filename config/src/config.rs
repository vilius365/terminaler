...
use terminaler_config_derive::ConfigMeta;
use terminaler_dynamic::{FromDynamic, ToDynamic};
use terminaler_input_types::{
...
pub struct Config {
    ...
    /// When true, launching a new terminaler instance will prefer
    ...
    /// For example, to have `terminaler` always run `top` by default,
    /// you'd use this:
    ///
    /// ```toml
    /// default_prog = ["top"]
    ...
    ///
    /// and you can set them in terminaler:
    ///
    /// ```toml
...
impl Default for Config {
    fn default() -> Self {
        ...
        Config::from_dynamic(
            &terminaler_dynamic::Value::Object(Default::default()),
            Default::default(),
        )
        .unwrap()
    }
...
impl Config {
    pub fn load() -> LoadedConfig {
        Self::load_with_overrides(&terminaler_dynamic::Value::default())
    }
    ...
    pub fn load_with_overrides(overrides: &terminaler_dynamic::Value) -> LoadedConfig {
        ...
        if let Some(path) = std::env::var_os("TERMINALER_CONFIG_FILE") {
            log::trace!("Note: TERMINALER_CONFIG_FILE is set in the environment");
    ...
    fn try_load(
        ...
        let (config, warnings) =
            terminaler_dynamic::Error::capture_warnings(|| -> anyhow::Result<Config> {
                ...
                std::env::set_var("TERMINALER_CONFIG_FILE", p);
                if let Some(dir) = p.parent() {
                    std::env::set_var("TERMINALER_CONFIG_DIR", dir);
                }
                Ok(cfg)
            });
        let cfg = config?;
    ...
    pub fn compute_extra_defaults(&self, config_path: Option<&Path>) -> Self {
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(terminaler_term::Intensity::Half),
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(false),
            intensity: Some(terminaler_term::Intensity::Bold),
            font: bold,
            ..Default::default()
        });
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(terminaler_term::Intensity::Normal),
            font: italic,
            ..Default::default()
        });
    ...
    pub fn apply_cmd_defaults(
        ...
        // and "$APPDIR" set in the terminaler process. These will be
        // propagated to the child processes. Since some apps (including
        // terminaler) use these variables to detect if they are running in
        ...
        cmd.env("TERM_PROGRAM", "Terminaler");
        cmd.env("TERM_PROGRAM_VERSION", crate::terminaler_version());
    }
}
...
pub(crate) fn compute_cache_dir() -> anyhow::Result<PathBuf> {
    ...
    Ok(crate::HOME_DIR.join(".local/share/terminaler"))
}
...
/// Behavior when the program spawned by terminaler terminates
#[derive(Debug, FromDynamic, ToDynamic, Clone, Copy, PartialEq, Eq, Default)]
...
impl FromDynamic for BoldBrightening {
    fn from_dynamic(
        ...
        options: terminaler_dynamic::FromDynamicOptions,
        ...
        match String::from_dynamic(value, options) {
            Ok(s) => match s.as_str() {
                ...
                s => Err(terminaler_dynamic::Error::Message(format!(
...
pub enum ImePreeditRendering {
    /// IME preedit is rendered by Terminaler itself
    #[default]
    Builtin,
