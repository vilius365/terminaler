...
// and terminaler_dynamic::Value.

/// Convert an mlua::Value to a terminaler_dynamic::Value
        ...
        Value::Table(t) => {
            ...
            if len > 0 {
                ...
                if is_seq {
                    ...
                    let mut obj = terminaler_dynamic::Object::default();
...
/// Convert a terminaler_dynamic::Value to an mlua::Value
...
/// Convert an mlua::Value to a Rust type via terminaler_dynamic::FromDynamic
...
/// Convert a Rust type to an mlua::Value via terminaler_dynamic::ToDynamic
...
static LUA_REGISTRY_USER_CALLBACK_COUNT: &str = "terminaler-user-callback-count";
...
pub fn get_or_create_sub_module<'lua>(
    ...
    let terminaler_mod = get_or_create_module(lua, "terminaler")?;
    let sub = terminaler_mod.get(name)?;
    match sub {
        Value::Nil => {
            let sub = lua.create_table()?;
            terminaler_mod.set(name, sub.clone())?;
            Ok(sub)
        }
        Value::Table(sub) => Ok(sub),
        wat => anyhow::bail!(
            "cannot register module terminaler.{name} as it is already set to a value of type {}",
            wat.type_name()
        ),
    }
}
        ...
        DynValue::Object(obj) => {
            match obj.get_by_str(&key) {
                None => {
                    ...
                    terminaler_dynamic::Error::warn(message);
                }
                Some(_dvalue) => {
...
/// terminaler specific config paths for lua modules, should
/// they choose to `require` additional code from their config.
///
/// A `terminaler` module is registered so that the script can
/// `require "terminaler"` and call into functions provided by
/// terminaler.  The terminaler module contains:
/// * `executable_dir` - the directory containing the terminaler
///   executable.  This is potentially useful for portable
///   installs on Windows.
/// * `config_dir` - the directory containing the terminaler
///   configuration.
/// * `log_error` - a function that logs to stderr (or the server
///   log file for daemonized terminaler).
/// * `target_triple` - the rust compilation target triple.
/// * `version` - the version of the running terminaler instance.
...
pub fn make_lua_context(config_file: &Path) -> anyhow::Result<Lua> {
    ...
    {
        let globals = lua.globals();
        // This table will be the `terminaler` module in the script
        let terminaler_mod = get_or_create_module(&lua, "terminaler")?;
        ...
        prefix_path(&mut path_array, &crate::HOME_DIR.join(".terminaler"));
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(path) = exe.parent() {
                terminaler_mod
                    ...
                    .context("set terminaler.executable_dir")?;
                if cfg!(windows) {
                    // For a portable windows install, force in this path ahead
                    // of the rest
                    prefix_path(&mut path_array, &path.join("terminaler_modules"));
                }
            }
        }
...
package.searchers[2] = function(module)
  ...
  if name then
    package.loaded.terminaler.add_to_config_reload_watch_list(name)
  end
  return orig(module)
end
        "#,
        ...
        terminaler_mod
            ...
            .context("set terminaler.config_file")?;
        terminaler_mod
            .set(
                "config_dir",
                config_dir
            ...
            .context("set terminaler.config_dir")?;

        lua.set_named_registry_value("terminaler-watch-paths", Vec::<String>::new())?;
        terminaler_mod.set(
            "add_to_config_reload_watch_list",
            lua.create_function(add_to_config_reload_watch_list)?,
        )?;

        terminaler_mod.set("target_triple", crate::terminaler_target_triple())?;
        terminaler_mod.set("version", crate::terminaler_version())?;
        terminaler_mod.set("home_dir", crate::HOME_DIR.to_str())?;
        terminaler_mod.set(
            "running_under_wsl",
        ...
        terminaler_mod.set("font", lua.create_function(font)?)?;
        terminaler_mod.set(
            "font_with_fallback",
            lua.create_function(font_with_fallback)?,
        )?;
        terminaler_mod.set("hostname", lua.create_function(hostname)?)?;
        // STRIPPED: terminaler_mod.set("action", luahelper::enumctor::Enum::<KeyAssignment>::new())?;
        terminaler_mod.set(
            "has_action",
        ...
        terminaler_mod.set("action_callback", lua.create_function(action_callback)?)?;
        terminaler_mod.set("exec_domain", lua.create_function(exec_domain)?)?;

        terminaler_mod.set("utf16_to_utf8", lua.create_function(utf16_to_utf8)?)?;
        terminaler_mod.set("split_by_newlines", lua.create_function(split_by_newlines)?)?;
        terminaler_mod.set("on", lua.create_function(register_event)?)?;
        terminaler_mod.set("emit", lua.create_async_function(emit_event)?)?;
        terminaler_mod.set("shell_join_args", lua.create_function(shell_join_args)?)?;
        terminaler_mod.set("shell_quote_arg", lua.create_function(shell_quote_arg)?)?;
        terminaler_mod.set("shell_split", lua.create_function(shell_split)?)?;
        ...
        // those values since terminaler was started
...
///
/// `terminaler.font("foo", {foreground="tomato"})`
/// yields:
...
///
/// `terminaler.font_with_fallback({"Operator Mono", "DengXian"})`
///
/// The second optional argument is a list of other TextStyle fields,
/// as described by the `terminaler.font` documentation.
...
/// This implements `terminaler.on`, whose goal is to register an event handler
/// callback.
...
/// `terminaler.emit` call.
///
/// ```lua
/// terminaler.on("event-name", function(arg1, arg2)
///   -- do something
///   return false -- if you want to prevent other handlers running
/// end);
///
/// terminaler.emit("event-name", "foo", "bar");
/// ```
pub fn register_event<'lua>(
    lua: &'lua Lua,
    (name, func): (String, mlua::Function),
) -> mlua::Result<()> {
    let decorated_name = format!("terminaler-event-{}", name);
...
const IS_EVENT: &str = "terminaler-is-event-emission";
...
pub fn add_to_config_reload_watch_list<'lua>(
    ...
    let mut watch_paths: Vec<String> = lua.named_registry_value("terminaler-watch-paths")?;
    watch_paths.extend_from_slice(&args);
    lua.set_named_registry_value("terminaler-watch-paths", watch_paths)?;
    Ok(())
}
...
assert(terminaler.emit('foo', 2) == false)

terminaler.on('bar', function (n, str)
...
assert(terminaler.emit('bar', 42, 'woot') == true)
"#,
            )
            .exec_async(),
        )?;
