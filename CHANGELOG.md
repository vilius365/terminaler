...
### Changed
- **Default attach is now a plain `tmux attach` in a 50% split** (picker Enter and sidebar row click), keeping your layout intact and letting tmux `bind -n` keys (e.g. F12 detach) work — control mode injects keys via `send-keys`, which bypasses tmux's key table entirely, so no root-table binding can ever fire there. Control mode (windows-as-native-tabs) remains available via the ⧉ icon on each sidebar session row. Session rows are styled as buttons so they read as clickable.
