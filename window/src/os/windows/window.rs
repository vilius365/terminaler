...
use std::rc::Rc;
use std::sync::Mutex;
use terminaler_color_types::LinearRgba;
use terminaler_font::FontConfiguration;
use terminaler_input_types::KeyboardLedStatus;
...
impl Window {
    fn create_window(
        ...
        let class = WNDCLASSW {
            ...
            cbWndExtra: 0,
            hInstance: h_inst,
            // FIXME: this resource is specific to the terminaler build and this should
...
unsafe fn update_title_font(hwnd: HWND) {
    ...
    if let Some(lf) = get_title_log_font(hwnd, hdc) {
        *font = terminaler_font::locator::gdi::parse_log_font(&lf, hdc).ok();
    }
